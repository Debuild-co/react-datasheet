...
var DataSheet = (function (_PureComponent) {
  ...
  function DataSheet(props) {
    ...
    _this.customHandleKeyDown = props.customHandleKeyDown;
  ...
  _createClass(DataSheet, [
    ...
    },
    {
      ...
      value: function componentDidMount() {
        ...
        var savedState = this.props.sheetSelectionState.current;

        if (
          (savedState || {}).start &&
          !_underscore2.default.isEmpty(savedState.start)
        ) {
          var start = savedState.start,
            end = savedState.end,
            shouldStartWithNewState = savedState.shouldStartWithNewState;
          ...
          if (this.props.data.length > 0) {
            ...
            var startI = Math.min(start.i, this.props.data.length - 1);
            var endI = Math.min(end.i, this.props.data.length - 1);
            var startJ = Math.min(start.j, this.props.data[0].length - 1);
            var endJ = Math.min(end.j, this.props.data[0].length - 1);
            var newSavedState = {
    ...
    },
    {
      ...
      value: function componentWillUnmount() {
        ...
        this.props.saveSheetSelectionState({ start: start, end: end });
      },
    },
    {
            ...
            var text = range(start.i, end.i)
              .map(function (i) {
                return range(start.j, end.j)
                  .map(function (j) {
                    var cell = data[i][j];
                    if (_this3.props.transformCopiedValue) {
                      return _this3.props.transformCopiedValue(cell, i, j);
                    } else {
                      return (
                        valueRenderer(cell, i, j) || dataRenderer(cell, i, j)
                      );
                    }
                  })
                  .join('\t');
        ...
        if (isEmpty(this.state.editing)) {
          ...
          if (onCellsChanged) {
            ...
            pasteData.forEach(function (row, i) {
              row.forEach(function (_value, j) {
                end = { i: start.i + i, j: start.j + j };
                var cell = data[end.i] && data[end.i][end.j];
                var value = _this4.props.parsePasteValue
                  ? _this4.props.parsePasteValue(_value, end.j)
                  : _value;
                if (!cell) {
    ...
    },
    {
      ...
      value: function handleKey(e) {
        ...
        var customKeyResult = this.customHandleKeyDown
        ...
        if (isEditing) {
          ...
          if (
            ...
            if (keyCode === _keys.ENTER_KEY) {
              func = function func() {
                return _this6.handleNavigate(e, { i: offset, j: 0 });
              };
            } else if (keyCode === _keys.TAB_KEY) {
              func = function func() {
                return _this6.handleNavigate(e, { i: 0, j: offset }, true);
              };
            }
            // setTimeout makes sure that component is done handling the event before we take over
            setTimeout(function () {
              func();
              _this6.dgDom && _this6.dgDom.focus();
            }, 1);
          }
        }
      },
    ...
    {
      key: 'listenToMouseDown',
      value: function listenToMouseDown() {
      ...
      value: function onScroll(_ref4) {
        ...
        if (this.shouldCalculate) {
          ...
          this.scrollSpeedInterval = setInterval(
            function () {
              if (_this7.shouldCalculate) {
                clearInterval(_this7.scrollSpeedInterval);
                _this7.scrollSpeedInterval = null;
                // console.log('reset interval');
              }
              _this7.shouldCalculate = true;
            }.bind(this),
    ...
    },
    {
      ...
      value: function render() {
        ...
        var _props6 = this.props,
          ...
          data = _props6.data,
          keyFn = _props6.keyFn,
          _props6$rowHeight = _props6.rowHeight,
          rowHeight = _props6$rowHeight === undefined ? 25 : _props6$rowHeight,
          _props6$height = _props6.height,
          height = _props6$height === undefined ? 300 : _props6$height,
          noRowsRenderer = _props6.noRowsRenderer,
          _props6$overscanRowCo = _props6.overscanRowCount,
          overscanRowCount =
            _props6$overscanRowCo === undefined ? 10 : _props6$overscanRowCo,
          onRowsRendered = _props6.onRowsRendered,
          loadingRenderer = _props6.loadingRenderer,
          _props6$speedThreshol = _props6.speedThreshold,
          speedThreshold =
            _props6$speedThreshol === undefined ? 100 : _props6$speedThreshol,
          _props6$speedUnit = _props6.speedUnit,
          speedUnit = _props6$speedUnit === undefined ? 200 : _props6$speedUnit,
          scrollTop = _props6.scrollTop,
          virtualRef = _props6.virtualRef;
            ...
            _react2.default.createElement(
              ...
              function (_ref5) {
                ...
                return _react2.default.createElement(_List2.default, {
                  ...
                  onScroll: _this8.onScroll.bind(_this8),
                  rowRenderer: function rowRenderer(_ref6) {
                    var i = _ref6.index,
                    ...
                    if (
                      isScrolling &&
                      (_this8.speedPer200ms || 0) > speedThreshold
                    ) {
                    ...
                    return _react2.default.createElement(
                      ...
                      { key: key, row: i, cells: row, style: style },
                      row.map(function (cell, j) {
                        var isEditing = _this8.isEditing(i, j);
                          ...
                          _extends(
                            {
                              ...
                              onRevert: _this8.onRevert,
                              onNavigate: _this8.handleKeyboardCellMovement,
                              onKey: _this8.handleKey,
                              selected: _this8.isSelected(i, j),
                              editing: isEditing,
                              clearing: _this8.isClearing(i, j),
